...
public class MapSelection {
    ...
    public void draw(Graphics g) {
        ...
        if (mapSelectionItems == null) {
            System.out.println("No map items to display");
            return;
        }

        String title = "Select a Map";
        int xLocation = (GameConstants.SCREEN_WIDTH - g.getFontMetrics().stringWidth(title)) / 2;
        g.setColor(Color.YELLOW);
        g.drawString(title, xLocation, 150);
        ...
        for (MapSelectionItem item : mapSelectionItems) {
            ...
            item.setLocation(new Point((GameConstants.SCREEN_WIDTH - width) / 2, item.getLocation().y));
