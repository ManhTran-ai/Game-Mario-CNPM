...
/**
 * Component ID: CLS-17
 * Purpose: Parses pixel-color map images and constructs the corresponding Map with all entities (bricks, enemies, Mario, end flag). Hidden Prize is randomly selected at map-creation time.
 * Owner: Member 4
 * Ref UML: OD, SD02, SD03
 * Derivation: Each colored pixel maps to a specific game entity; SurpriseBrick contains a randomly generated Prize.
 */
class MapCreator {
    ...
    /**
     * Method ID: MTH-001
     * Extracts all sprite regions from the sprite sheet for use during map creation.
     */
    ...
    /**
     * Method ID: MTH-002
     * Loads a map image and instantiates game entities based on pixel colors.
     * @param mapPath  Resource path to the map image.
     * @param timeLimit Initial countdown time.
     * @return The constructed Map, or null if the image cannot be loaded.
     */
        ...
        for (int x = 0; x < mapImage.getWidth(); x++) {
            for (int y = 0; y < mapImage.getHeight(); y++) {
                ...
                if (currentPixel == ordinaryBrick) {
                    ...
                    ((Turtle) enemy).setLeftImage(turtleLeft);
    ...
    /**
     * Method ID: MTH-003
     * Randomly selects one of four prize types with equal probability.
     * @return SuperMushroom (rand==0), FireFlower (rand==1), OneUpMushroom (rand==2), or Coin (rand>=3).
     */
